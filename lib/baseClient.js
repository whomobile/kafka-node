  ...
  operation.attempt(currentAttempt => {
    ...
    getTopicsFromKafka(topics, function (error, kafkaTopics) {
      ...
      if (left.length === 0) {
        ...
        self.loadMetadataForTopics(topics, function (error, resp) {
          if (error) {
            logger.error('Topics created metadata refresh error: ', error);
          }
          logger.debug('Topics created metadata refreshed: ', Object.keys(resp[1].metadata));
        });
